...
function setSpace {
	count=$1
	touch setSpace.txt
	#echo n integers from $1-$2 range inclusive, in a file
	for (( i=0; i <= $(($2-$1)); i++ )); do
		echo $count >> setSpace.txt
		count=$(($count+1))
	done
...
function getRandom {
	...
	if echo "$1$2$3" | grep -Eq '[0-9]{9}'; then
		searchSpace=$(($2-$1))
		if [[ $searchSpace -lt $1 ]]; then
			echo -e "Usage - rand: ./thumbnail.sh rand <pick> <start> <end>"
			echo -e "\n\t <pick> being an integer lesser than the inclusive range from <start> to <end>"
			echo -e "\n\t <start> and <end> of the form xxxx; where x is a digit"
			exit 104

			#Iterate until <pick> or maximum possible pictures have been retrieved within range
			#Search space has holes, therefore we need to use sample without replacement
			
			#First set search space by inserting each possible file within range
			#setSpace $2 $3
			#getSample	
		fi
	else
